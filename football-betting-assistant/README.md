...
组合报告应先给比赛大纲 / 今日赛程确认，再给数据来源总览和逐场分析。每场分析至少包含数据依据、基础面、数学模型、概率倾向、比分覆盖、等级和风险；之后再给赔率/盘口摘要、每场结论汇总、组合相关性检查、分档参考购买方案、稳健/搏冷组合候选、更多组合候选，以及 Pass 或排除项说明。

四串一或比分组合报告默认使用分档参考购买方案：

- `2 元档`：1 注，每场 1 个方向或最集中比分。
- `16 元档`：8 注，默认 2 元/注，常见结构 `1 x 2 x 2 x 2`。
- `32 元档`：16 注，默认结构 `2 x 2 x 2 x 2`。
- `48 元档`：24 注，默认结构 `3 x 2 x 2 x 2`。
- `稳健方向单`：以胜平负、让球、大小球或总进球方向为主，避免过度依赖精确比分。
- `基础比分覆盖`：覆盖泊松模型最集中的比分区间。
- `增强比分覆盖`：给最高方差场次增加一个合理比分路径。
- `搏冷/高赔率小单`：只列为小概率高方差参考，不作为核心方案。
- `更多组合候选`：只列作备选，不要求用户全部购买。

报告必须把金额、注数和组合结构分开写清楚，避免把 16 注误写成 16 元。
